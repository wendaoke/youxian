...
public interface CommentMapper {
	...
	@Select("SELECT * FROM comment WHERE type = #{type} and item_id =#{item_id} order by update_time desc LIMIT #{start} , #{length} ")	
	...
	@Select("SELECT count(*) FROM comment WHERE type = #{type} and item_id =#{item_id} order by update_time desc   ")	
	Integer findCommentCount(@Param("type") Integer type,@Param("item_id") String itemId);
	
	@Insert("insert into comment (id,type,item_id,content,commentator_id,commentator_name,commentator_headimg) "
